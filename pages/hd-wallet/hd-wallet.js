angular.module("app").component("hdWalletPage", {
...
function HdWalletPageController(lodash, allNetworks) {
  ...
  const METHOD_NONE = 0,
    METHOD_PBKDF2 = 1,
    METHOD_COINOMI = 2;
  const BITCOIN = lodash.find(allNetworks, ["label", "LTC (Litecoin, legacy, BIP32/44)"]);
  ...
  vm.bips = [
    {
      id: 0,
      label: "BIP32 (Bitcoin Core)",
      bip: "32",
      hasCoinType: false,
      path: "m/account'/change'/index",
      base58Prefixes: [
    ...
    {
      id: 2,
      label: "BIP49 (SegWit P2SH-P2WPKH)",
      bip: "49",
      hasCoinType: true,
      path: "m/49'/coin'/account'/change/index",
      base58Prefixes: [
    ...
    {
      id: 4,
      label: "BIP86 (Native SegWit v1 bech32m P2TR)",
      bip: "86",
      hasCoinType: true,
  ...
  vm.fromMnemonic = function () {
    ...
    if (vm.passphrase) {
      if (vm.strenghtening.id === METHOD_PBKDF2) {
        pw = bitcoin.pbkdf2
          .pbkdf2Sync(bitcoin.Buffer.from(vm.passphrase, "utf8"), PBKDF2_SALT, PBKDF2_ROUNDS_APP, PBKDF2_HMACLEN, "sha512")
          .toString("hex");
