angular.module("app").component("walletImportPage", {
...
function WalletImportPageController(lodash, bitcoin, allNetworks, Buffer) {
  ...
  const BITCOIN_TESTNET = lodash.find(allNetworks, ["label", "LTC (Litecoin Testnet, legacy, BIP32/44)"]);
  const SCHEMES = [
    {
  ...
  vm.fromMnemonic = function () {
    ...
    if (vm.passphrase) {
      if (vm.strenghtening.id === METHOD_PBKDF2) {
        pw = bitcoin.pbkdf2
          .pbkdf2Sync(Buffer.from(vm.passphrase, "utf8"), PBKDF2_SALT, PBKDF2_ROUNDS_APP, PBKDF2_HMACLEN, "sha512")
          .toString("hex");
  ...
  vm.getResultAsDump = function (rootNode, basePath, network) {
    ...
    let str = `# Wallet dump created by litecoin toolkit
# * Created on ${date}
